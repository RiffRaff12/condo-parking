export function createDb(client) {
  return {
    async createRequest({ requester_id, requester_phone, requester_name, requester_unit, start_datetime, end_datetime }) {
      const { data, error } = await client
        .from('requests')
        .insert({ requester_id, requester_phone, requester_name, requester_unit, start_datetime, end_datetime, status: 'open' })
        .select()
        .single()
